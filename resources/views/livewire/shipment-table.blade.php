<div>
    <div class="mx-auto" x-data="{ loading: true, loadingTable: true }"
        ...
        {{ $shipment->status == 'shipped' ? 'bg-blue-500' : ($shipment->status == 'processing' ? 'bg-yellow-500' : ($shipment->status == 'delivered' ? 'bg-green-500' : ($shipment->status == 'failed' ? 'bg-red-500' : 'bg-gray-500'))) }}">
                                    ...
                                    <div x-data="{ openModal: false }">
                                        ...
                                        <div x-show="openModal" x-transition
                                            ...
                                            <div @click.away="openModal = false"
                                                ...
                                                <!-- Body -->
                                                <form method="POST" s
                                                    action="{{ route('shipment.update', $shipment->id) }}"
                                                    class="space-y-3">
                                                    @csrf
                                                    @method('PUT')
                                                    <div>
                                                        <div class="mb-4 text-left">
                                                            ...
                                                            <select name="status"
                                                                ...
                                                                required>
                                                                <option value="processing" {{ $shipment->status ==
                                                                    'processing' ? 'selected' : '' }}>Processing
                                                                </option>
                                                                <option value="shipped" {{ $shipment->status ==
                                                                    'shipped' ? 'selected' : '' }}>Shipped</option>
                                                                <option value="delivered" {{ $shipment->status ==
